...
HE_face* Mesh3D::InsertFace(vector<HE_vert* >& vec_hv)
{
	int vsize = vec_hv.size();

	if (pfaces_list_ == NULL)
	{
		pfaces_list_ = new vector<HE_face*>;
	}
...
void Mesh3D::WriteToOBJFile(const char* fouts)
{
	...
	fout<<"#####"<<endl;
	fout<<"# OBJ File Generated by MiniMeshFrame"<<endl;
	fout<<"#####"<<endl;
	fout.precision(6);
	...
	{
		fout<<"v "<< scientific <<(*viter)->position_.x() 
...
void Mesh3D::SetBoundaryFlag(void)
{
	for (EDGE_ITER eiter = pedges_list_->begin(); eiter!=pedges_list_->end(); eiter++)
	{
		...
		{
			(*eiter)->boundary_flag_ = BOUNDARY;
			(*eiter)->ppair_->boundary_flag_ = BOUNDARY;
			(*eiter)->pvert_->boundary_flag_ = BOUNDARY;
			(*eiter)->ppair_->pvert_->boundary_flag_ = BOUNDARY;
			(*eiter)->ppair_->pface_->boundary_flag_ = BOUNDARY;
		}
	}
}
...
void Mesh3D::BoundaryCheck()
{
	for (VERTEX_ITER viter=pvertices_list_->begin(); viter!=pvertices_list_->end(); viter++)
	{
		if ((*viter)->isOnBoundary())
		{
			...
			int deg = 0;
			while (edge->pface_!=NULL && deg<(*viter)->degree_)
			{
...
void Mesh3D::ComputePerFaceNormal(HE_face* hf)
{
	...
	// these two vectors stand for two edges of the face
...
void Mesh3D::ComputePerVertexNormal(HE_vert* hv)
{
	...
	// the normal of an inner vertex is average of the normals of all faces connected to it
	{
		int iterNum = 0;
		do 
		{
			iterNum++;
			if (iterNum > hv->degree_)
			{
				/*hv->set_position(hv->position() * 1.1f);*/
				std::cout << "    iterNum > hv->degree : " << hv->id_ << "\n";
				break;
			}
			//hv->normal_ = hv->normal_ + edge->pface_->normal_;
			Vec3f  p = edge->pvert_->position_,
				q = edge->pnext_->pvert_->position_,
				r = edge->pprev_->pvert_->position_;
			Vec3f  n = (q-p) ^ (r-p);
			hv->normal_ = hv->normal_ + n;
			edge = edge->ppair_->pnext_;
			// let the pointer "edge" traverse each edges emanating from the vertex "p"
...
void Mesh3D::ComputeAvarageEdgeLength(void)
{
	...
	for (int i=0; i<num_of_half_edges_list(); i++)
	{
		...
		aveEdgeLength += (v0->position_ - v1->position_).length();
	}
...
void Mesh3D::GetNeighbors(const size_t vertid, std::vector<size_t>& neighbors)
{
	HE_edge *start,*edge;
	edge = start = get_vertex(vertid)->pedge_;
	neighbors.clear();
	do 
	{
		neighbors.push_back(edge->pvert_->id_);
		edge = edge->ppair_->pnext_;
	} while ( edge != start && edge != NULL );
}

void Mesh3D::GetVertexOnEdge( std::vector<size_t>& vertex_on_edge )
{
	vector<size_t> tmp_list_;
	int i = 0, j,k;
	vertex_on_edge.clear();
	// start from a point
	while( !get_vertex(i)->isOnBoundary() )
		i++;
	vertex_on_edge.push_back(i);
	j = i;
	bool end = false;
	while( !end )
	{
		GetNeighbors(j,tmp_list_);
		k = 0;
		while( !get_vertex(tmp_list_[k])->isOnBoundary() )
			k++;
		j = tmp_list_[k];
		if( j == i )         // come back to the start point i
			end = true;
		else
			vertex_on_edge.push_back(j);
	}
}

int Mesh3D::GetFaceId(HE_face* face)
{
	return !face ? -1 : face->id_;
}

void Mesh3D::ResetFaceSelectedTags(SelectTag tag)
{
	for (int i=0; i<num_of_face_list(); i++)
		get_face(i)->selected_ = tag;
}

void Mesh3D::ResetVertexSelectedTags(SelectTag tag)
{
	for (int i=0; i<num_of_vertex_list(); i++)
		get_vertex(i)->selected_ = tag;
}
...
int Mesh3D::GetSelectedVrtId()
{
	...
	for (int i=0; i<num_of_vertex_list(); i++)
	{
		if (get_vertex(i)->selected_ == SELECTED)
		{
			return i;
		}
	}
